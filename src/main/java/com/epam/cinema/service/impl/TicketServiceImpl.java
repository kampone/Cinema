...
public class TicketServiceImpl implements TicketService {
    ...
    public void bookTicketWithIdForUser(Long ticketId, Long userId) {
        ticketRepository.bookTicketForUser(ticketRepository.getAllTickets().stream().filter(it ->ticketId.equals(it.getId())).findFirst().orElse(null), userId);
    }

    @Override
    public void unbookTicketWithId(Long ticketId, Long userId) {
        ticketRepository.unbookTicketForUser(ticketRepository.getAllTickets().stream().filter(it ->ticketId.equals(it.getId())).findFirst().orElse(null), userId);
    }
    @Override
